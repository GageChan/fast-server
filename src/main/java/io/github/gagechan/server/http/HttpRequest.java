...
public class HttpRequest {
    ...
    */
    public HttpRequest(FullHttpRequest request) {
        ...
        parseHeaders();
        parseParam(decoder);
        // add config into attribute
        addConfigToAttr();
    }

    private void addConfigToAttr() {
        AppConfigProperties config = AppConfigProperties.getInstance();
        Map<String, Object> configs = config.getConfigs();
        configs.forEach(this::addAttribute);
        addAttribute("path", this.path);
    }
